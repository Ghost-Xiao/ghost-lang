...
新特性
- **面向对象编程支持**，包括类定义、实例化、成员方法和属性
- **面向对象语法**：class、this、super、实例化等关键字
- **继承**，支持类的继承关系和 super 关键字
- **try-catch-finally 异常处理**，提供结构化的异常处理机制
- **throw 语句**，支持抛出内置异常和自定义异常
- **Error 类体系**，包括 Error 基类和多种内置错误类型
- **time 模块**，提供时间和日期处理功能
- **VS Code 插件**，提供 Ghost 语言的开发支持
  - 语法高亮
  - 代码补全（关键字、内置函数、错误类型、模块）
  - 静态错误检查（未闭合字符串、不匹配括号等）
  - 支持 `.gh` 文件扩展名
  - 构建脚本支持 VS Code 插件打包
  - 发布包包含 VS Code 插件的 .vsix 文件
- **math 模块新增函数**
  - `factorial(n)`：计算阶乘 n!
...
改进
- 完整的内置类支持，包括 Int、Float、String、List、Map 等
...
- [内置模块说明](docs/modules.md) - 内置模块的详细文档
